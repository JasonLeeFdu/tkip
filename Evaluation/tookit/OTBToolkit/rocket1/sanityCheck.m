...
overWrite = false;
resPathBase = fullfile('/home/winston/workSpace/PycharmProjects/tracking/TrackingGuidedInterpolation/Evaluation/results','SanityCheck');
datasetBase = fullfile('/home/winston/Datasets/Tracking/Original/OTB50');
...
for idxVideo=1:length(videosList)
    ...
    for idxTrk=1:numTrk     %For every tracker
        ...
        resultsOri = {};
        resultsAdv = {};
        disp([ 'sanity check' ' --- ' num2str(idxTrk) '_' t.name ', ' num2str(idxVideo) '_' videosList(idxVideo).name])       
        
        %%%
        saveOri =  fullfile(resPathBaseTrk,resOriFileSaveName);
        saveAdv =  fullfile(resPathBaseTrk,resAdvFileSaveName);
        %%%
        [resAdv,fpsAdv] = ModuleSimpleRun(t.name,imgSet,init_rect);
...
for idxDownSampleType = 1:length(downSampleTypeSet)
    ...
    for idxMetricType = 1:length(metricTypeSet)
        ...
        % error_OPE_Ori   overlap_OPE_Ori   error_OPE_Adv    overlap_OPE_Adv 
