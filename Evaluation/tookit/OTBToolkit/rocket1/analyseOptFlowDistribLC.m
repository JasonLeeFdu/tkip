...
for i = 1:length(vnames)    %every clip
    ...
    for j = startFr:endFr-1
       ...
       [h,w] = size(OptFlowAmplitude);
       pos = gtBoxes(j - startFr + 1,:);
       ys = max(1,pos(2)); ye = min(ys + pos(4) - 1,h);
       xs = max(1,pos(1)); xe = min(xs + pos(3) - 1,w);
       croppedOptFlowAmplitude = OptFlowAmplitude(ys:ye,xs:xe);
       resBar = resBar + hist(croppedOptFlowAmplitude(:),bin);
    end
    fprintf('\n');
end
