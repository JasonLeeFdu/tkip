...
overWrite = false;
resPathBase = fullfile('/home/winston/workSpace/PycharmProjects/tracking/TrackingGuidedInterpolation/Evaluation/results/ComparativeTest','BothRects');
...
for idxVideo=2:2:length(videosList) %% Here to do the paralell things
    ...
    for idxTrk=1:numTrk     %For every tracker
        ...
        str0 = ['[resAdv ,InterpBboxAdv,MDEGArr,th,fpsAdv] = run_' t.name '_' 'ADV3_5_3' '(imgSet,init_rect,' num2str(BEST_TH) ');'];
        eval(str0);                       
        results = {}; 
        res = struct;
