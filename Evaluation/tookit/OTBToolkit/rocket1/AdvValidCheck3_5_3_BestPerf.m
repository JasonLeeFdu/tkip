...
%%  就是用来跑光流增强算法的！ --- 光流框 + 上一帧的框 都用，然后不用插帧算法。 不设阈值。混合选择制
...
overWrite = false;
MAX_TRAIL_TIMES = 10;
resPathBase = fullfile('/home/winston/workSpace/PycharmProjects/tracking/TrackingGuidedInterpolation/Evaluation/results',strcat('BothRectsBlend_BestPerf',num2str(MAX_TRAIL_TIMES)));
...
for idxVideo=  1:length(videosList)% 对于每一个视频(此处可以使用多进程)  
    ...
    for trailTimes = 1:MAX_TRAIL_TIMES
        ...
        for idxTrk=1:numTrk     %For every tracker
            ...
            %%%
            str0 = ['[resAdv ,InterpBboxAdv,MDEGArr,th,fpsAdv] = run_' t.name '_' 'ADV3_5_3' '(imgSet,init_rect,' num2str(-1) ');'];
            eval(str0);                       
            results = {}; 
            res = struct;
