...
MAX_TRAIL_TIMES = 3;
resPathBase = fullfile('/home/winston/workSpace/PycharmProjects/tracking/TrackingGuidedInterpolation/Evaluation/results',strcat('ETA',num2str(MAX_TRAIL_TIMES)));
...
<<<<<<< HEAD
for idxVideo=77:7:length(videosList)% 对于每一个视频(此处可以使用多进程)  
=======
for idxVideo=99:-7:1%length(videosList)% 对于每 一个视频(此处可以使用多进程)  
>>>>>>> 3d1543bd3801eaea3ebd08aeafc0838f33713238
    ...
    for trailTimes = 1:MAX_TRAIL_TIMES
        ...
        for idxTrk=1:numTrk     %For every tracker
            ...
            %%%
            str0 = ['[resAdv ,InterpBboxAdv,MDEGArr,th,fpsAdv] = run_' t.name '_' 'ADV3_7_2' '(imgSet,init_rect,' num2str(-1) ');'];
            eval(str0);                       
            results = {}; 
            res = struct;
