...
%%  就是用来跑光流增强算法的！ --- 只用光流，然后不用插帧算法。 不设阈值
...
overWrite = false;
resPathBase = fullfile('/home/winston/workSpace/PycharmProjects/tracking/TrackingGuidedInterpolation/Evaluation/results/ComparativeTest','OnlyFlowedRect');
...
for idxVideo=1:4:length(videosList) %% Here to do the paralell things
    %% get the imgSet
    ...
    for idxTrk=1:numTrk     %For every tracker
        ...
        str0 = ['[resAdv ,InterpBboxAdv,MDEGArr,th,fpsAdv] = run_' t.name '_' 'ADV3_5_2' '(imgSet,init_rect,' num2str(BEST_TH) ');'];
        eval(str0);                       
        results = {}; 
        res = struct;
