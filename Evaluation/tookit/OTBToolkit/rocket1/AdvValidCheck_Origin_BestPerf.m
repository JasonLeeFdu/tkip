...
overWrite = false;
MAX_TRAIL_TIMES = 5;
resPathBase = fullfile('/home/winston/workSpace/PycharmProjects/tracking/TrackingGuidedInterpolation/Evaluation/results',strcat('OriginalAlg_BestPerf',num2str(MAX_TRAIL_TIMES)));
...
<<<<<<< HEAD
for idxVideo=  6:6:length(videosList)% 对于每一个视频(此处可以使用多进程)  
=======
for idxVideo=  1:8:length(videosList)% 对于每一个视频(此处可以使用多进程)  
>>>>>>> b99efe968ab8d764ca86b8580a56d45167cd0439
    ...
    for trailTimes = 1:MAX_TRAIL_TIMES
        ...
        for idxTrk=1:numTrk     %For every tracker
            ...
            %%%
            str0 = ['[resAdv ,fpsAdv] = run_' t.name '(imgSet,init_rect );'];
            eval(str0);                       
            results = {}; 
            res = struct;
