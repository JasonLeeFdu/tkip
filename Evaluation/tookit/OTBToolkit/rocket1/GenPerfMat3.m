function GenPerfMat3(seqs, trackers, evalType, perfMatPath, trkResPath,nameTrkAll)
...
for idxSeq=1:length(seqs)  
    ...
    results = {};
    load([rpAll s.name '_' t.name '_Adv.mat']);
    if isempty(results)
        res = resultsAdv{1};
        results{end+1}=res;
    ...
    for idxTrk=1:numTrk  
        t = trackers{idxTrk};
        if strcmp(nameTrkAll,'')
            load([rpAll s.name '_' t.name '.mat'])
        else
            load([rpAll s.name '_' t.name '_' nameTrkAll '.mat']) 
        end
...
if strcmp(nameTrkAll,'')
    ...
    dataName1=[perfMatPath 'aveSuccessRatePlot_' num2str(numTrk) 'alg_overlap_' evalType  '_'   nameTrkAll '.mat'];
    save(dataName1,'aveSuccessRatePlot','nameTrkAll');
    dataName2=[perfMatPath 'aveSuccessRatePlot_' num2str(numTrk) 'alg_error_' evalType  '_'   nameTrkAll '.mat'];
    aveSuccessRatePlot = aveSuccessRatePlotErr;
    save(dataName2,'aveSuccessRatePlot','nameTrkAll');
end