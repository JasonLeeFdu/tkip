...
if strcmp(additionalNameTag,'')
    outputPath =  fullfile(conf.BASE_PATH,strcat('/Evaluation/results/vidvis',strcat('_',additionalNameTag))); %%%%%% GET IT MODIFIED
else
    outputPath =  fullfile(conf.BASE_PATH,strcat('/Evaluation/results/vidvis')); %%%%%% GET IT MODIFIED
end
isAllVideos = true;
isAllMethods = true;
isOverwrite = false;
...
for idxVideo = 1:length(testVideoSet)
    ...
    for idxTrk = 1:length(methodName)
        ...
        Visualization1(choiceRes,imgSet, bundleRes, resVideoName,append); 
    end
end
