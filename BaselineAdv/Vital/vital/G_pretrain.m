...
%pretrain_asdn
%   net_fc,  net_G,   pos_data is feature
...
for i=1:numel(net.layers)  % net_G and announce its momentums
...
net_fc.layers = net_fc.layers(1:end-1); % remove the softmaxloss layer
...
for k=1:9
    ...
    for i=1:nBatches
        ...
        % blacken the k-th place of the featuremap
        batch(col,row,:,:)=0;
        if(opts.useGpu)
        ...
        % feature map to score f (unstardized logits)
        f = gather(res(end).x) ;
        if ~exist('feat','var')
        ...
        % with all input '1'--- as if we already know the label
        prob_k(k)=sum(Y(1,1,1,:)); % try to probe with position has the lowest '1' prob
end
...
% extract positive batches || GLOBALLY SHUFFLE 全局大随机
train_pos = [];
...
for t=1:opts.maxiter
    ...
    labels(col,row,:)=0; % pick out the feat map region which contribute minimum to correct recog
