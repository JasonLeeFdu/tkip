...
%     fprintf('extract batch %d/%d...\n',i,nBatches);
    ...
    batch = ims(:,:,:,opts.batchSize_test*(i-1)+1:min(end,opts.batchSize_test*i)); % ArrXXX(min(end))
    if(opts.useGpu) % put the input to gpu
    ...
    f = gather(res(end).x) ;
    if ~exist('feat','var')
        feat = zeros(size(f,1),size(f,2),size(f,3),n,'single'); %init first feat
    end
